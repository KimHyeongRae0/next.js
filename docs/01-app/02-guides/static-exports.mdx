...
When your host supports it, prefer rewriting only document navigations to `_fallback.html`. Internal Next.js data requests such as missing `*.txt` and `*.json` fallback artifacts should stay normal 404s instead of being rewritten to HTML. This keeps the critical path shorter and avoids turning an internal probe into an extra HTML response.

For example, on Nginx you can preserve 404s for missing data assets while still rewriting document navigations:
...
If your host can only apply one broad catch-all rewrite, `/_fallback.html` will still work, but missing internal data requests may also be rewritten to HTML. The page will still recover, but you may see extra fallback probes in the network waterfall.
